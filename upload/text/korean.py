...
def normalize(text):
    ...
    text = re.sub('[a-zA-Z]+', normalize_upper, text.upper())
    text = re.sub('[a-zA-Z]+', normalize_upper2, text)
    text = re.sub('[-=+,#/*]','',text)
...
def normalize_upper2(text):
    text= text.group(0)
    if all([char.islower() for char in text]):
        return "".join(upper_test[char] for char in text)
    else:
        return text
...
if __name__ == "__main__":
    ...
    test_normalize("234s DY A 아 KM 키 ABSOULte a b c d e g abc wwoe m cm 35km ")
    test_normalize("Hello 3242  * -")
